 ...
 *  4차 피드백
 *  - 발생 가능한 문제
 *      세션 1이 트랜잭션을 시작하고 생성된 IDX로 데이터를 insert하는 동안 아직 커밋을 수행하지 않았는데,
 *      세션 2에서 동시에 같은 insert를 호출하게 되면 IDX의 데이터의 원자성이 깨지게 된다.
 *  - 해결법
 *      데이터를 조회하여 변경하는 로직을 수행할 때에는 select for update 구문을 사용한다.
 *
 */
