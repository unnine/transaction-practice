 ...
 * - 원인은?
 * 현재 트랜잭션이 서비스에 걸려있음
 * 승인 로직과 시험 로직은 물리 트랜잭션이 분리되어 있지 않기 때문에 예외 발생 시 물리 트랜잭션을 롤백시키지 않고
 * rollbackOnly 설정을 변경 함 (예외 처리 여부와 무관함)
 * 작업이 끝나고 트랜잭션이 종료 될때 트랜잭션매니저가 포함한 논리트랜잭션들의
 * rollbackOnly 설정을 확인하고 하나라도 롤백처리 시 전체 롤백을 수행함
 *
 *
 * 모든 논리 트랜잭션이 커밋되야 물리트랜잭션 모두 커밋
 * 하나 라도 롤백되면 모두롤백
 *
 *
 */
