 ...
 * 2) 동시 사용자가 같은 로직을 요청 가능한 경우, 다른 사용자의 처리가 실패해도 정상 요청한 사용자는 처리되야할 때
 *
 * === 24.03.21 피드백 ===
 * 내용: 2번 상황 수정
 *
 * - Propagation.REQUIRES_NEW가 필요한 상황은?
 * 1) 부분 롤백: 예상되는 예외 사항에서 트랜잭션을 분리하여 로직을 반드시 처리해야할 때 필요
 * 즉, 하나의 메서드에서 여러 개의 작업을 수행하고, 그 중 일부는 실패할 수 있는 경우가 있음.
 * 이 경우에 Propagation.REQUIRES_NEW를 사용하여 각 작업을 별도의 트랜잭션으로 처리하면, 작업이 실패했을 때 다른 작업에 영향을 주지 않고 실패한 작업만 롤백되기 때문에 해당 옵션을 사용
 */
